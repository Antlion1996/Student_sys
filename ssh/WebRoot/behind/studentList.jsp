...
function doEdit(id) {
	frm.action = "<%=basePath%>studentAction!editStudent.action?id=" + id;
}

function dofind() {
	document.forms[0].action = "<%=basePath%>studentAction!getStudentById.action";
	document.forms[0].submit;
}

function dodelete(id) {
	frm.action = "<%=basePath%>studentAction!deleteStudent.action?id=" + id;
	if (!confirm("确认要删除？")) {
...
function getPwd(id) {
	frm.action = "<%=basePath%>studentAction!getStudentPwd.action?id="+id;
}
</script>
		...
		<center>
			...
			<s:form action="" method="post" name="frm">
				<table width="80%" border="1 solid" cellpadding="10" cellspacing="0">
					...
					<s:iterator id="students" value="students" status="index">
						<tr
